...
class BLContentModel: NSObject {
    ...
    init(dict: [String: Any]) {
        ...
        if let array = dict["top_cmt"] as? [[String : Any]], array.count > 0 {
            top_cmt = BLCommentModel(dict: array.first!)
        }
    }
    ...
    var cellHeight: CGFloat {
        get {
            if _cellHeight == 0 {
                ...
                var size = CGSize(width: kScreen_width - BLCellTextMargin * 2, height: 10000)
                ...
                _cellHeight = BLCellHeaderHeight + size.height + BLCellFooterHeight + 2 * BLCellTextMargin
                
                if type == ContentType.picture.rawValue {
                    // 图片显示出来的宽度
                    let pictureW: CGFloat = kScreen_width - BLCellTextMargin * 2;
                    // 显示显示出来的高度
                    var pictureH: CGFloat = pictureW * height / width;
                    
                    if pictureH >= BLCellIamgeMaxH {
                        pictureH = BLCellImageBreakHeight
                        isBigPicture = true
                    }
                    
                    pictureFrame = CGRect(x: BLCellTextMargin, y: BLCellHeaderHeight + size.height + 2 * BLCellTextMargin , width: pictureW, height: pictureH)
                    _cellHeight += (pictureH + BLCellTextMargin)
                    ...
                    voiceFrame = CGRect(x: BLCellTextMargin, y: BLCellHeaderHeight + size.height + 2 * BLCellTextMargin , width: pictureW, height: pictureH)
                    _cellHeight += (pictureH + BLCellTextMargin)
                    ...
                    videoFrame = CGRect(x: BLCellTextMargin, y: BLCellHeaderHeight + size.height + 2 * BLCellTextMargin , width: pictureW, height: pictureH)
                    _cellHeight += (pictureH + BLCellTextMargin)
                }
